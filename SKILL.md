...
- **搜索引擎容错**：调研阶段（阶段 0.5）如果 `web_search` 出现 429/503 等配额错误，**立即切换到 `multi-search-engine` 技能**（通过 `web_fetch` 爬取 Google/Bing 等搜索引擎），不要在配额不足的情况下继续强行调用。
...
3. **内化动态参考样板（防 Token 爆炸与风格统一）**：拿到人类赐予的个性标签后，首先探测当前工作空间(Workspace)根目录的 `team_personas/` 目录下积累的已有成品画像。如果该目录下已生成的画像文档数量 ≥3 份，则从中随机抽取 3 份进行阅读（保持本公司团队基因的绝对延续）；否则（初始拓荒阶段），为保证首批生成者的极高质量，你必须静默前往【本技能自带的】 `best-practice/` 文件夹下随机抽取 3 份以 `2` 开头的成熟大样作为排版审美参考。参透它们的语言脉络、结构审美与叙事张力。
   - ⚠️ **`team_personas/` 目录只存在于工作空间(Workspace)根目录**，不要在本技能目录下创建此目录。
4. **大融合与重塑出卷**：遵循你在步骤 2 读过的核心骨牌《`20-指南`》，融合步骤 1 的《公司和雇主基石》土壤，装载步骤 3 的顶级专家武器库，**并将其毫无违和感地依附在用户刚刚钦定的个性人设（女、28岁等）之上**。在参考模板的加持下，生造出一份立体的、具有这家公司特有口吻属性的、属于雇主私人的霸道数字员工**最终形态画像文档**（强烈要求应用正规商用命名法，例如：`[名字]_[对标水位]-[岗位名称]_画像.md`，例：`Emily_Staff-数据分析专家_画像.md`）。交付入库至工作空间根目录的 **`team_personas/`** 目录（不存在则新建）。
