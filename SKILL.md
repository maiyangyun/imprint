...
- **Sub-agent strategy for 04 document (v0.6)**: The 04 scenario document must use multi-subagent batch generation. Divide 18-22 scenarios into batches of ≤3 scenarios each. Each subagent receives only the relevant responsibility/competency/knowledge fragments (NOT full 01+02+03). Main session assembles batch outputs into the final document and runs cross-validation. See imprint-engine Phase 5 for full protocol.
...
4. **Grand Fusion & Soul Package Forging**: Following the core blueprint `templates/20-Guide` you read in Step 2, fuse the Step 1 《Company and Employer Cornerstone》 soil, load the Step 3 top-tier expert arsenal, **and seamlessly graft it all onto the personality persona the user just designated (female, 28 years old, etc.)**. With the template's reinforcement, distribute all fused content to the Soul Package files according to the mapping rules in the **Soul Package Specification**.
   ...
   **Forging Process (v0.4 Two-Tier Architecture)**:
   ...
   - **Portability check (v0.6)**: Before copying the profile package, verify that 01-04 documents do NOT contain hard-coded references to the current employer's company name, product name, or specific business metrics. The profile package must be reusable — if the same Agent is later assigned to a different project or organization, the professional capability profile should remain valid without modification. Organization-specific context is provided by the cornerstone documents (01-团队, 02-公司, 03-雇主) and AGENTS.md, not by the profile package.
